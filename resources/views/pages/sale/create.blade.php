            ...
            <div class="col-12 mb-4">
                <div class="bs-stepper wizard-icons wizard-icons-example mt-2">
                    <div class="bs-stepper-header">
                        ...
                        @endif
                        @if(isset($Invoicecharges_perm) && $Invoicecharges_perm->view == 1 || (isset($Payment_perm) && $Payment_perm->view == 1))
                            <div class="line">
                                <i class="mdi mdi-chevron-right"></i>
                            </div>
                        ...
                        @endif
                        @if(isset($refund_perm) && $refund_perm->view == 1 || (isset($chargeBack_perm) && $chargeBack_perm->view == 1))
                            <div class="line">
                                <i class="mdi mdi-chevron-right"></i>
                            </div>
                               ...
                               {{-- Client Details --}}
                                                    ...
                                                    <label class="switch switch-lg">
                                                        ...
                                                        <input type="checkbox" class="switch-input" name="sale_status"
                                                            ...
                                                            $sale->status == 1) checked
                                                                @if($user->role_id == 1 || in_array($user->role->name, ['Customer Support', 'Manager Customer Support']))
                                                                {{-- <p>{{ $user->role_id }}</p> --}}
                                                                    @readonly(false)
                                                                @else
                                                                    @readonly(true)
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="form-floating form-floating-outline">
                                                            <select id="payment_type" name="payment_type" class="select2 form-select" data-allow-clear="true">
                                                                ...
                                                                <option value="Partials Payment">Partial Payment</option>
                                ...
                                @if(isset($refund_perm) && $refund_perm->view == 1 || (isset($chargeBack_perm) && $chargeBack_perm->view == 1))
                                    <div id="refund-chargeback" class="content">
                                        @if(isset($refund_perm) && $refund_perm->view == 1)
                                            <div class="content-header mb-3">
                                            ...
                                            <form id="refund_form" method="POST" action="{{ route('refund.store') }}">
                                                    ...
                                                    <div class="row g-4">
                                                        <input type="hidden" name="lead_id" value="{{ $lead->id }}">
                                                        <input type="hidden" name="invoice_id" id="refund_invoice_id">
                                                        <div class="col-md-6 select2-primary">
                                                            <div class="form-floating form-floating-outline">
                                                        ...
                                                        <div class="col-md-6 select2-primary">
                                                            <div class="form-floating form-floating-outline">
                                                                <select id="select-payment" name="payment_id" class="select2 form-select">
                                                                    <option value="">Select Invoice</option>
                                                                    @if(isset($sale) && isset($payments))
                                                                    @foreach ($payments as $payment)
                                                                    <option value="{{ $payment->id }}">{{ $payment->invoice->invoice_number }} (${{ $payment->amount }}) {{ $payment->created_at->format('F j, Y') }}</option>
                                                                    @endforeach
                                                                    @endif
                                                                </select>
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="form-floating form-floating-outline">
                                                                <select id="refund_merchant" name="merchant_id" class="select2 form-select" data-allow-clear="true">
                                                                    <option value="">Please Select</option>
                                                                    @if(isset($mehchant) && count($mehchant) > 0)
                                                                    @foreach ($mehchant as $item)
                                                                    <option value="{{$item->id }}">{{ $item->name }}</option>
                                                        ...
                                                        <div class="col-12">
                                                            ...
                                                            <div class="card-body">
                                                                    <textarea class="form-control ckeditor" style="border-radius: 0px" name="refund_reason"
                                        ...
                                        @endif
                                        {{-- ChargeBack --}}
                                        @if(isset($chargeBack_perm) && $chargeBack_perm->view == 1)
                                            <div class="content-header mb-3">
                                            ...
                                            <form id="chargeback_form" method="POST" action="{{ route('chargeback.store') }}">
                                                ...
                                                <div class="row g-4">
                                                    <input type="hidden" name="lead_id" value="{{ $lead->id }}">
                                                    <input type="hidden" id="chargeBack_invoiceId" name="invoice_id">
                                                    <div class="col-md-6 select2-primary">
                                                        <div class="form-floating form-floating-outline">
                                                            <select id="select-payment-chargeback" name="payment_id" class="select2 form-select">
                                                                <option value="">Select Invoice</option>
                                                                @if(isset($sale) && isset($payments))
                                                                @foreach ($payments as $payment)
                                                                <option value="{{ $payment->id }}">{{ $payment->invoice->invoice_number }} (${{ $payment->amount }}) {{ $payment->created_at->format('F j, Y') }}</option>
                                                    ...
                                                    <div class="col-12">
                                                        ...
                                                        <div class="card-body">
                                                                <textarea class="form-control ckeditor" style="border-radius: 0px" name="chargeBack_reason"
                                                                    cols="30" rows="5"></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-12 text-right m-auto py-3">
        ...
        $('#select-payment').change(function (e) {
            ...
            if(refund_type == "")
            {
                ...
                $.ajax({
                    ...
                    data: {payment_id: payment_id, refund_type: refund_type},
                    success: function (response) {
                        var payment = response.payment;
                        ...
                        if(refund_type == "Full"){
                            ...
                            $('#refund_invoice_id').val(payment.invoice.id);
                            $('#refund_merchant').val(payment.merchant_id).trigger('change');
...
<script>
    $(document).ready(function () {
        $('#select-payment-chargeback').change(function (e) {
            e.preventDefault();
            // alert("sdfs");
            let payment_id = $(this).val();
            ...
            console.log(refund_type);
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('front.getchargeBack') }}",
                    data: {payment_id: payment_id},
                    success: function (response) {
                        var payment = response.payment;
                        var total = payment.amount;

                            $('#chargeBack_invoiceId').val(payment.invoice.id);
                            $('#merchant-chargeback').val(payment.merchant_id).trigger('change');
